...
int main(int argc, char **argv)
{
    ...
    MatrixXd Ain(numConstr,numVar); Ain << 1.0, -1.0,
                                           2.0, -2.0;
    VectorXd Bin(numConstr);        Bin << 1.0, 5.0;
    ...
    if (solver->setProblem(H, f, Ain, Bin))
        std::cout << "CPLEX solver problem set" << std::endl;
    else
    ...
    if (solver->solveProblem(result_CPLEX, optimizerStatus))
    {
        ...
        std::cout << "Optimal cost: " << result_CPLEX.transpose()*H*result_CPLEX+f << std::endl;
        std::cout << "Inequality constraint Ain*x-Bin (should be <=0): [ " << (Ain*result_CPLEX-Bin).transpose() << " ]" << std::endl;
    }
    else
